...
SALE_COMMON_AREA_URL = "https://sale.591.com.tw/home/house/detail/137/1590.html"
NEWHOUSE_COMMON_AREA_URL = "https://newhouse.591.com.tw/456/detail"


def _load_fixture(name: str) -> str:
    return (FIXTURES / "conversation_listings" / name).read_text(encoding="utf-8")
...
class TestCommonAreaRatio:
    def test_sale_common_area_from_dom(self) -> None:
        html = _load_fixture("sale_common_area.html")
        result = parse_listing_detail(
            html, canonical_url=SALE_COMMON_AREA_URL, listing_type="sale"
        )
        assert result.common_area_ratio == 0.348
        assert result.common_area_ratio_source == "591 DOM 公設比"

    def test_sale_no_common_area(self) -> None:
        html = _load_fixture("sale_no_common_area.html")
        result = parse_listing_detail(
            html, canonical_url=SALE_URL, listing_type="sale"
        )
        assert result.common_area_ratio is None
        assert result.common_area_ratio_source is None

    def test_newhouse_common_area_from_jsonld(self) -> None:
        html = _load_fixture("newhouse_common_area_jsonld.html")
        result = parse_listing_detail(
            html, canonical_url=NEWHOUSE_COMMON_AREA_URL, listing_type="newhouse"
        )
        assert result.common_area_ratio == 0.325
        assert result.common_area_ratio_source == "591 JSON-LD"

    def test_malformed_348_percent_rejected(self) -> None:
        html = _load_fixture("sale_malformed_common_area.html")
        result = parse_listing_detail(
            html, canonical_url=SALE_URL, listing_type="sale"
        )
        assert result.common_area_ratio is None
    ...
    def test_common_area_from_meta_description(self) -> None:
        html = """<html><head>
<title>青埔三房 - 591售屋網</title>
<meta name="description"
 content="桃園市中壢區住宅出售：位於青禾社區，公設比28.5%，總價2000萬">
</head><body>
<span class="info-price-num-2">2,000</span>
<div class="info-addr-content">
  <span class="info-addr-key">地址</span>
  <span class="info-addr-value-text">桃園市中壢區青埔路</span>
</div>
<a class="community-info-a">青禾社區</a>
</body></html>"""
        result = parse_listing_detail(
            html, canonical_url=SALE_URL, listing_type="sale"
        )
        assert result.common_area_ratio == 0.285
        assert result.common_area_ratio_source == "591 meta description"

    def test_negative_common_area_rejected(self) -> None:
        html = """<html><head>
<title>Test</title>
</head><body>
<div class="info-addr-content">
  <span class="info-addr-key">公設比</span>
  <span class="info-addr-value-text">-5%</span>
</div>
<div class="info-addr-content">
  <span class="info-addr-key">地址</span>
  <span class="info-addr-value-text">桃園市中壢區</span>
</div>
</body></html>"""
        result = parse_listing_detail(
            html, canonical_url=SALE_URL, listing_type="sale"
    ...
    def test_non_numeric_common_area_rejected(self) -> None:
        html = """<html><head>
<title>Test</title>
</head><body>
<div class="info-addr-content">
  <span class="info-addr-key">公設比</span>
  <span class="info-addr-value-text">N/A</span>
</div>
<div class="info-addr-content">
  <span class="info-addr-key">地址</span>
  <span class="info-addr-value-text">桃園市中壢區</span>
    ...
    def test_sale_has_community_name(self) -> None:
        html = _load_fixture("sale_common_area.html")
        result = parse_listing_detail(
            html, canonical_url=SALE_COMMON_AREA_URL, listing_type="sale"
        )
        assert result.community_name == "青禾社區"
