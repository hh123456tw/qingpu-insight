...
def _automl_patch_shared_feature_environment(
    monkeypatch: pytest.MonkeyPatch,
    service_fixture: tuple,
    market: str = "resale",
) -> dict:
    """Patch the AutoML pipeline to simulate community features for shared feature testing."""
    import qingpu_insight.model_training_service as mts
    from qingpu_insight.community_registry import CommunityRegistry
    from qingpu_insight.model_analysis import FeatureExperiment, SharedFeatureExperimentResult
    from qingpu_insight.model_features import COMMUNITY_FEATURE_COLUMNS, FEATURE_COLUMNS
    from qingpu_insight.model_training import (
        ModelFitSpec,
    )

    mock_registry = CommunityRegistry(
        _data=pd.DataFrame({
            "community_id": ["comm_a", "comm_b"],
            "canonical_name": ["Comm A", "Comm B"],
            "aliases": ["", ""],
            "station_code": ["A17", "A18"],
            "address_patterns": ["Street A", "Street B"],
            "twd97_x": [276000.0, 277000.0],
            "twd97_y": [2767000.0, 2768000.0],
            "completion_year": [2015, 2016],
            "source_notes": ["test", "test"],
        }),
        _version="test-version",
    )

    original_build = mts.build_model_frame

    def _build_model_frame_with_community(frame, market):
        result = original_build(frame, market)
        for col in COMMUNITY_FEATURE_COLUMNS:
            if col not in result.columns:
                result[col] = "unknown" if col == "community_known" else 0.0
        return result

    monkeypatch.setattr(mts, "build_model_frame", _build_model_frame_with_community)
    monkeypatch.setattr(mts, "build_community_feature_snapshot", lambda *a, **kw: {})

    _empty_split = {
        "row_count": 0, "date_min": "", "date_max": "",
        "median_target": 0.0, "station_proportions": {},
        "building_type_proportions": {},
    }
    _dq = {
        "special_relationship_excluded": 0,
        "non_market_subject_excluded": 0,
        "ambiguous_registration_note_count": 0,
    }
    _fake_diag = {
        "station_counts": {}, "missing_rates": {}, "monthly_summary": [],
        "building_type_summary": [], "feature_extreme_rates": {},
        "evaluation_error_summary": [], "top_residuals": [],
        "data_quality": _dq,
        "split_summary": {
            "train": dict(_empty_split),
            "calibration": dict(_empty_split),
            "test": dict(_empty_split),
        },
    }
    monkeypatch.setattr(mts, "build_resale_diagnostics", lambda *a, **kw: _fake_diag)

    original_gate = mts.ModelTrainingService._evaluate_shared_feature_release_gate

    def patched_gate(self_, cm, bm, bt, hsf, **kw):
        if not hsf:
            return original_gate(self_, cm, bm, bt, hsf, **kw)
        return {
            "shared_mae_improved_2pct": True,
            "shared_mape_not_worsened": True,
            "shared_stations_within_limit": True,
            "shared_backtests_passed": True,
            "shared_pi_coverage_ok": True,
            "shared_validation_ok": True,
            "shared_feature_gate_passed": True,
        }

    monkeypatch.setattr(
        mts.ModelTrainingService, "_evaluate_shared_feature_release_gate", patched_gate
    )
    ...
    def _make_trial(trial_number, metrics, mae):
        from qingpu_insight.automl_search import AutoMLTrialResult
        tests.append(1)
        return AutoMLTrialResult(
            trial_number=trial_number, state="completed", fit_spec=fit_spec,
            estimator=None, metrics=metrics, overall_mae=mae,
            overall_mape=float(metrics.get("overall", {}).get("mape", 0)),
            station_mape=dict(station_mape), calibration_passed=True,
            reason_codes=(), duration_seconds=1.0,
        )

    from qingpu_insight.automl_search import AutoMLSearchResult, rank_trials, shortlist_trials
    trials = tuple(_make_trial(i, dummy_metrics, 45000 + i * 1000) for i in range(3))
    ranked = rank_trials(trials)
    shortlisted = shortlist_trials(ranked)
    search_result = AutoMLSearchResult(
        market=market, budget_name="quick", budget_seconds=300,
        max_trials=12, seed=42, elapsed_seconds=10.0, stopped=False,
        trials=trials, ranked_trials=ranked, shortlisted_trials=shortlisted,
    )
    monkeypatch.setattr(mts, "run_automl_search", lambda *a, **kw: search_result)

    shared_cols = FEATURE_COLUMNS + COMMUNITY_FEATURE_COLUMNS
    fe_base = FeatureExperiment(
        name="baseline_v3", feature_columns=FEATURE_COLUMNS,
        selected_model="ridge", metrics=dummy_metrics, candidate_errors={},
    )
    fe_comm = FeatureExperiment(
        name="community", feature_columns=shared_cols,
        selected_model="ridge", metrics=dummy_metrics, candidate_errors={},
    )
    mock_shared_result = SharedFeatureExperimentResult(
        calibration_experiments=(fe_base, fe_comm),
        locked_feature_set_name="community",
        locked_feature_columns=shared_cols,
        selection_reason="locked community (MAE=45000.0, MAPE=8.50, features=17)",
    )
    monkeypatch.setattr(mts, "run_shared_feature_experiments", lambda split: mock_shared_result)

    def _fake_backtests(*a, **kw):
        return [{"cutoff_date": "2025-01-01", "train_max_date": "2024-06-01",
                 "test_min_date": "2025-01-01", "source_max_date": "2025-12-31",
                 "passed": True, "stations_within_limit": True,
                 "candidate_metrics": {"overall": {"mae": 45000}},
                 "baseline_metrics": {"overall": {"mae": 50000}}}]

    monkeypatch.setattr(mts, "run_annual_backtests", _fake_backtests)

    return {"mock_registry": mock_registry, "shared_cols": shared_cols}


class TestAutoMLSharedFeatureOrchestration:
    def test_automl_resale_has_shared_feature_experiment(
        self, tmp_path, market_parquet, automl_plan, automl_service_fixture, monkeypatch
    ):

        service, jobs, registry, output_store = automl_service_fixture
        test_inst = TestAutoMLOrchestration()
        test_inst._patch_automl_pipeline(monkeypatch, passing=True, market="resale", n_trials=3)
        _automl_patch_shared_feature_environment(monkeypatch, automl_service_fixture)
    ...
    def test_automl_resale_stores_shared_feature_evidence_in_artifact(
        self, tmp_path, market_parquet, automl_plan, automl_service_fixture, monkeypatch
    ):
    ...
    def test_automl_guided_registry_loading(
        self, tmp_path, market_parquet, monkeypatch
    ):
        csv_dir = tmp_path / "data" / "reference"
        csv_dir.mkdir(parents=True, exist_ok=True)
        csv_path = csv_dir / "qingpu_communities.csv"
        csv_line = (
            "community_id,canonical_name,aliases,station_code,"
            "address_patterns,twd97_x,twd97_y,completion_year,source_notes\n"
            "comm_a,Comm A,,A17,Street A,276000.0,2767000.0,2015,test\n"
        )
        csv_path.write_text(csv_line)
        ...
        def patched_exists(self):
            if str(self).endswith("qingpu_communities.csv"):
                return True
